...
//
// typedef struct
void printNode(CSGNode root, int level);

void printDesc(CSGType root, int level)
{
    if(root == NULL)
        return;
    int i=0;
...
void printNode(CSGNode root, int level)
{
    ...
    switch (root->class) {
        ...
        case 6:
            printf("Inst\n");
            printNode(root->x, level+1);
            printf("%c\n", root->op);
            printNode(root->y, level+1);
            break;
    }
    ...
    printDesc(root->type, level+1);
    printNode(root->nxt, level+1);
    printf("************************\n");
    printNode(root->next, level);
}
// ****************************************************************
